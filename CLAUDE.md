...
- **Skills** (`/skills`, `SkillsTablePage`): The core domain — every `claude_code.skill_activated` event ingested via OTLP is bucketed by skill name and plugin. Skills are derived from events (no manual create); the `skills` table is upserted on ingest. Status (`to_review` / `approved` / `removed` / `denied` / `ignored`) is inherited from the owning plugin's marketplace status — a marketplace marked `denied` (or `approved` / `ignored`) cascades that status down to its plugins and skills via `computePluginStatus` and `propagateStatusFromPlugins`. New skills land in `to_review` by default; bundled built-ins (`skillSource === "bundled"`) display as `approved` via the `defaultBundledStatus` helper. `denied` is purely informational — it labels entities you no longer want active but does not gate anything on the developer's machine.
