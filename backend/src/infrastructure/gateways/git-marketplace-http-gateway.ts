...
async function fetchPluginSkills(params: {
	...
	try {
		if (host === "github") {
			const url = `https://api.github.com/repos/${owner}/${repo}/contents/${pluginPath}/skills?ref=${branch}`;
		...
		if (host === "gitlab") {
			...
			const path = encodeURIComponent(`${pluginPath}/skills`);
			const url = `https://gitlab.com/api/v4/projects/${encoded}/repository/tree?path=${path}&ref=${branch}`;
		...
		if (host === "bitbucket") {
			const url = `https://api.bitbucket.org/2.0/repositories/${owner}/${repo}/src/${branch}/${pluginPath}/skills/`;
			const res = await fetch(url, { headers, signal: AbortSignal.timeout(10_000) });
			if (!res.ok) return [];
